...
our_bfgs <- function(theta,f,...,tol=1e-5,fscale=1,maxit=100){
  ...
  else
  {
    ...
    while (updated_gradient %*% QN_step < c_2 * current_gradient %*% QN_step | f(updated_theta,...)>=f(current_theta,...) )
...
our_bfgs(theta=c(-1,2),rb,getg=TRUE)