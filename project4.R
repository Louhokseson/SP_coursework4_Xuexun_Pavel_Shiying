...
our_bfgs <- function(theta,f,...,tol=1e-5,fscale=1,maxit=140){
  ...
  while (N_steps<=maxit)
  {
    ...
    if (is.infinite(current_gradient) | is.infinite(current_function_value))
    {
      stop('Something wrong with your parameters. It goes to infinity in your input function. Please have check')
    }
  ...
  {
    stop("The algorithm reached the maximum iteration but it didn't converge.") 
  }
...
res=our_bfgs(theta=c(-10,10),rb,getg=TRUE)
print(res)
